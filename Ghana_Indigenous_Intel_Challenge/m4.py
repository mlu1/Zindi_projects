...
def parse_time_features(df, time_col='prediction_time'):
    ...
    if time_col in df.columns:
        ...
        #df['pred_date'] = dt.dt.month.astype('str')
...
def engineer_features(df_:pd.DataFrame) -> pd.DataFrame:
    preds = "predicted_intensity"
    df_["predicted_intensity"] = df_["predicted_intensity"].fillna(value = -999)
    df_["predicted_intensity"] = df_["predicted_intensity"].bfill().bfill()
    for step_ in tqdm([1,2,3,4,7,21,30]):
        df_[f"Target_lag_{step_}"] = df_["predicted_intensity"].shift(periods = step_).bfill().bfill()
    ...
    for lag in range(1,2+ 1):
        df_[f'{preds}_Lead_{lag}'] = df_["predicted_intensity"].shift(-lag)
    ...
    dtf = DatetimeFeatures(
        variables = ["prediction_time"],
        drop_original = True,
        features_to_extract = ["month", "quarter", "year", "week", "day_of_month", "day_of_year"]
    )
    gcf = CyclicalFeatures(
        variables = ["prediction_time" + "_" + i for i in ["month", "day_of_year", "day_of_month"]]
    )
